from LMS.common import Session # 로그인한 member 객체
...
class ScoreService :
    ...
    def load(cls):
        ...
        try :
            with conn.cursor() as cursor :
                # 커서 객체는 db 연결 성공 시 연결 정보를 가지고 있음
                cursor.execute('SELECT count(*) as cnt FROM scores')
                # sql문 실행
                count = cursor.fetchone()['cnt']
                # 실행 결과를 1개 가져와 count 변수에 넣음
                print(f"시스템 : 현재 등록된 성적 수는 {count} 개입니다.")
        except :
            print("ScoreService.load() 실행 오류 발생!!!!")

        finally :
            conn.close() # db 연결 정보를 받는다.

    @classmethod
    def run(cls) : # 성적 처리용 주 실행 메서드
        cls.load()
        ...
        while True :
            ...
            if sel == "1" and member.role in ("manager", "admin"):
                cls.add_score() # 성적 추가 / 수정

            elif sel == "2":
                cls.view_my_score() # 성적 확인

            elif sel == "3" and member.role == "admin":
                cls.view_all() # 성적 전체 확인 (관리자 전용)
    ...
    def add_score(cls) : # admin이나 manager가 입력 가능
        ...
        try :
            with conn.cursor() as cursor :
                # 학생 존재 확인 (pk -> fk에 대한 문제 해결용) :
                # 부모 테이블에 자료가 있어야 자식 테이블에 자료를 넣을 수 있다.
                ...
                temp_score = Score(member_id = student['id'], kor = kor, eng = eng, math = mat)
    ...
    def view_my_score(cls) :
        ...
        try :
            with conn.cursor() as cursor : # 연결 성공 시 true
                ...
                data = cursor.fetchone() # data 에는 member db 정보가 담김

            if data : # 데이터가 있으면
                s = Score.from_db(data) # dict 타입의 객체를 s에 넣음
                # 도메인 클래스의 __init__ 에는 uid 정보가 없으므로 세션 정보를 활용해 출력
    ...
    def view_all(cls) :
        ...
        try :
            with conn.cursor() as cursor :
                # members와 scores를 JOIN 하여 아이디(uid)와 성적을 함께 가져옴
                sql = """
                        SELECT m.uid, s.* \
                        FROM scores s \
    ...
    @classmethod
    def print_score(cls, s, uid) : # 개인 성적 출력과 전체 성적 출력도 가능 (메서드 : 동작 -> 재활용 가능)
